...
const DEPTS=["Audit","Tax","GST","Finance","Billing","Operations","HR","IT","Management"];
...
const mapUser=r=>r?({id:r.id,cid:r.company_id,companyId:r.company_id,name:r.name,email:r.email,role:r.role,avatar:r.avatar,dept:r.dept,balance:parseFloat(r.balance)||0,reimbursable:parseFloat(r.reimbursable)||0,delegateTo:r.delegate_to||null}):null;
...
const mkPolicy=()=>({
  ...
  departmentBudgets:{Audit:50000,Tax:40000,GST:30000,Finance:60000,Billing:25000,Operations:45000,HR:20000,IT:55000,Management:80000},
...
const SA={id:"sa1",name:"Super Admin",email:"admin@hisaabhub.in",password:"superadmin@123",role:"superadmin",avatar:"SA"};
const STORAGE_KEY="hisaabhub_v7_db";
const SESSION_KEY="hisaabhub_v7_sess";
...
class ErrorBoundary extends Component{
  ...
  componentDidCatch(e,i){console.error("HisaabHub Error:",e,i);}
  render(){
...
const Logo=({width=200,dark=false})=>(
  <svg width={width} viewBox="0 0 680 220" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="192" y="98"  fontFamily="'Noto Sans Devanagari',sans-serif" fontWeight="800" fontSize="72" fill={dark?"#fff":"#1a1a1a"} letterSpacing="-1">हिसाब</text>
    <text x="196" y="130" fontFamily="'Sora',sans-serif" fontWeight="300" fontSize="28" fill="#7ED957" letterSpacing="14">HUB</text>
    <line x1="192" y1="149" x2="570" y2="149" stroke="#7ED957" strokeWidth="0.8" opacity="0.4"/>
    <text x="192" y="171" fontFamily="'Sora',sans-serif" fontWeight="400" fontSize="13" fill="#7ED957" opacity="0.75" letterSpacing="3">by RB</text>
  </svg>
);
...
function Login({onLogin,DB}){
  ...
  const attempt=async(em,pw)=>{
    ...
    try{
      if(SB_ENABLED){
        ...
        // Auth state listener in Root handles the redirect
      } else {
        // ── localStorage demo mode ─────────────────────────────────────────
        await new Promise(r=>setTimeout(r,500));
        const e=em.toLowerCase().trim();
        if(e===SA.email&&pw===SA.password){onLogin(SA,null);return;}
        for(const cid of Object.keys(DB)){
          const co=DB[cid];
          if(co.meta.status==="Suspended")continue;
          const u=co.users.find(u=>u.email.toLowerCase()===e);
          if(u){if(u.password!==pw){throw new Error("Incorrect password.");}onLogin(u,co.meta);return;}
        }
  ...
  const googleLogin=async()=>{
    ...
    try{
      if(SB_ENABLED){
        ...
        attempt("rushabh@rbshah.in","admin@123");
      }
  ...
  const DEMOS=[["🔑 Super Admin","admin@hisaabhub.in","superadmin@123"],["👔 Manager","rushabh@rbshah.in","admin@123"],["👤 Employee","janvi@rbshah.in","janvi@111"],["🚗 Roger Motors","rajesh@rogermotors.in","rajesh@123"]];
  ...
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(145deg,${DARK} 0%,#162e0d 45%,#0c1f08 100%)`,display:"flex",fontFamily:FB,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",padding:"60px 64px",position:"relative",zIndex:1}}>
        ...
        {["⚡  Auto-approve within policy limits","🤖  Batch OCR · camera capture · handwritten bills","📊  Tally XML · Zoho Books · GSTR-2A export","🌐  Multi-company with fully isolated data","📴  Offline mode — syncs when reconnected"].map(f=>(
        ...
        {SB_ENABLED&&<div style={{marginTop:20,padding:"8px 12px",background:"rgba(126,217,87,0.1)",border:"1px solid rgba(126,217,87,0.3)",borderRadius:8,fontSize:11,color:G}}>🟢 Connected to Supabase — production mode</div>}
        {!SB_ENABLED&&<div style={{marginTop:20,padding:"8px 12px",background:"rgba(251,191,36,0.1)",border:"1px solid rgba(251,191,36,0.3)",borderRadius:8,fontSize:11,color:"#fbbf24"}}>🟡 Demo mode — add VITE_SUPABASE_URL to enable full backend</div>}
      </div>
      {/* Right form */}
      <div style={{width:460,display:"flex",flexDirection:"column",justifyContent:"center",padding:40,position:"relative",zIndex:1}}>
        <div style={{background:"rgba(255,255,255,0.04)",backdropFilter:"blur(24px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:18,padding:"36px 32px",animation:"fadeUp .4s ease"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><div style={{flex:1,height:1,background:"rgba(255,255,255,0.1)"}}/><span style={{color:"rgba(255,255,255,0.25)",fontSize:11}}>or email</span><div style={{flex:1,height:1,background:"rgba(255,255,255,0.1)"}}/></div>
          <div style={{marginBottom:12}}>
            <label style={{display:"block",fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.4)",letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>Work Email</label>
            <input type="email" value={email} onChange={e=>{setEmail(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&attempt(email,pass)} placeholder="you@company.in" style={inp}/>
          </div>
          <div style={{marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              ...
              <span style={{color:G,fontSize:11,cursor:"pointer"}}>Forgot?</span>
            </div>
            <div style={{position:"relative"}}>
              <input type={showPw?"text":"password"} value={pass} onChange={e=>{setPass(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&attempt(email,pass)} placeholder="••••••••" style={{...inp,paddingRight:44}}/>
          ...
          <button onClick={()=>attempt(email,pass)} disabled={busy||!email||!pass} style={{marginTop:12,width:"100%",padding:"13px",background:busy||!email||!pass?"rgba(126,217,87,0.3)":G,border:"none",borderRadius:10,color:"#fff",fontFamily:FB,fontWeight:700,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,transition:"all .2s"}}>
          ...
          {!SB_ENABLED&&<div style={{marginTop:16,padding:"12px",background:"rgba(126,217,87,0.05)",border:"1px solid rgba(126,217,87,0.12)",borderRadius:10}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Demo accounts</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
              {DEMOS.map(([role,em,pw])=>(
                <div key={em} onClick={()=>{setEmail(em);setPass(pw);setErr("");}} style={{cursor:"pointer",padding:"6px 8px",background:"rgba(255,255,255,0.04)",borderRadius:7,border:"1px solid rgba(255,255,255,0.07)"}}>
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginBottom:1}}>{role}</div>
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600}}>{em}</div>
                </div>
              ))}
            </div>
          </div>}
        </div>
        <div style={{marginTop:14,textAlign:"center",fontSize:10,color:"rgba(255,255,255,0.2)"}}>© 2026 HisaabHub by RB · Privacy · Terms</div>
      </div>
    </div>
  );
}
  ...
  return(
    <div style={{display:"flex",minHeight:"100vh",fontFamily:FB,background:"#f0f4f8"}}>
      ...
      <div style={{width:220,background:"#1e2736",display:"flex",flexDirection:"column",padding:"20px 12px",position:"sticky",top:0,height:"100vh"}}>
        ...
        {SB_ENABLED&&<div style={{background:"rgba(126,217,87,0.15)",border:"1px solid rgba(126,217,87,0.3)",borderRadius:6,padding:"4px 8px",marginBottom:10,fontSize:9,color:G,fontWeight:600,textAlign:"center"}}>🟢 Supabase Connected</div>}
      ...
      <div style={{flex:1,padding:"28px 32px",overflow:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
          <div><h1 style={{fontFamily:FD,fontSize:24,fontWeight:700,color:"#1e2736"}}>HisaabHub Control Centre</h1><p style={{color:MUTED,fontSize:13,marginTop:3}}>Manage all companies, users and billing</p></div>
          {tab==="companies"&&<Btn onClick={()=>setMdl({type:"newCo"})}>＋ New Company</Btn>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:22}}>
          {[{l:"Companies",v:allCo.length,i:"🏢",c:"#6366f1"},{l:"Active",v:allCo.filter(c=>c.meta.status==="Active").length,i:"✅",c:"#16a34a"},{l:"Total Users",v:SB_ENABLED?Object.values(userCounts).reduce((a,b)=>a+b,0):Object.values(DB).reduce((s,c)=>s+c.users.length,0),i:"👥",c:G},{l:"Backend",v:SB_ENABLED?"Live":"Demo",i:"🗄️",c:SB_ENABLED?"#16a34a":"#f59e0b"}].map((s,i)=>(
        ...
        {tab==="companies"&&!loading&&<Card><table style={{width:"100%"}}>
          ...
          <tbody>{allCo.map(co=>(
            <tr key={co.meta.id} className="rh">
              ...
              <td style={{fontWeight:600}}>{co.meta.maxUsers}</td>
...
function SaUsers({DB,userCounts}){
  ...
  return(<Card><table style={{width:"100%"}}><thead><tr><th>Name</th><th>Email</th><th>Company</th><th>Role</th><th>Dept</th></tr></thead>
    <tbody>{rows.map(u=><tr key={u.id} className="rh"><td><div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:28,height:28,borderRadius:"50%",background:GL,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,color:GD,fontSize:10}}>{u.avatar||inits(u.name)}</div><span style={{fontWeight:600,fontSize:13}}>{u.name}</span></div></td><td style={{color:MUTED,fontSize:12}}>{u.email}</td><td style={{fontSize:11}}>{u.coName||(u.companies?.name)||u.company_id||""}</td><td style={{textTransform:"capitalize",color:MUTED,fontSize:12}}>{u.role}</td><td style={{color:MUTED,fontSize:12}}>{u.dept}</td></tr>)}</tbody>
  </table></Card>);
}
...
function CompanyApp({user,meta,DB,setDB,onLogout,sbReload}){
  ...
  const[coData,setCoData]=useState(SB_ENABLED?null:DB[cid]);
  const[loadingData,setLoadingData]=useState(SB_ENABLED);
  ...
  // When DB changes in localStorage mode, sync coData
  useEffect(()=>{
    if(!SB_ENABLED)setCoData(DB[cid]);
  },[DB,cid]);
  ...
  if(loadingData||!co)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f5faf3",fontFamily:FB}}>
      <div style={{textAlign:"center"}}><div style={{width:40,height:40,border:`3px solid ${GM}`,borderTopColor:G,borderRadius:"50%",animation:"spin .8s linear infinite",margin:"0 auto 14px"}} /><div style={{color:MUTED,fontSize:13}}>{sbError?`Error: ${sbError}`:"Loading your workspace…"}</div>{sbError&&<Btn onClick={loadFromSB} style={{marginTop:12,padding:"8px 16px",fontSize:12}}>Retry</Btn>}</div>
    </div>
  );
  ...
  // Push notification on load
  useEffect(()=>{
    if(!co)return;
    const n=co.claims.filter(c=>c.status==="Pending").length;
    if(n>0&&canApprove)sendPush("HisaabHub",`${n} claim${n!==1?"s":""} await your approval`);
  },[]);
  ...
  const submitClaim=useCallback(async(form)=>{
  ...
  const handleDecision=async(claimId,decision,remarks="")=>{
    ...
    if(SB_ENABLED){
      ...
      emailAlert(getUser(claim.empId)?.email||"",`Claim ${decision} — HisaabHub`,`${claimId} for ${fmt(claim.amount)} has been ${decision.toLowerCase()}.${remarks?" Remarks: "+remarks:""}`);
      await loadFromSB();
    } else {
  ...
  const addUserToSB=async(userData,password)=>{
    ...
    const{data:authData,error:authErr}=await supabase.auth.signUp({email:userData.email,password,options:{data:{name:userData.name}}});
    if(authErr)throw new Error(authErr.message);
    const{error:uErr}=await supabase.from("users").insert({id:authData.user.id,company_id:cid,name:userData.name,email:userData.email,role:userData.role||"employee",avatar:inits(userData.name),dept:userData.dept||"Operations",balance:userData.balance||0,reimbursable:0});
    if(uErr)throw new Error(uErr.message);
    await loadFromSB();
  ...
  const exportCSV=()=>{
    ...
    const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));a.download=`HisaabHub_${cid}_${today()}.csv`;a.click();toast("📊 CSV exported");
  };
  const exportTally=()=>{
  ...
  return(
    <div style={{display:"flex",minHeight:"100vh",fontFamily:FB,background:"#f5faf3"}}>
      ...
      <div style={{width:sidebar?222:60,background:DARK,display:"flex",flexDirection:"column",padding:sidebar?"20px 12px":"20px 8px",position:"sticky",top:0,height:"100vh",overflow:"hidden",transition:"width .22s ease",flexShrink:0}}>
        ...
        {sidebar&&SB_ENABLED&&<div style={{background:"rgba(126,217,87,0.1)",border:"1px solid rgba(126,217,87,0.2)",borderRadius:6,padding:"3px 8px",marginBottom:8,fontSize:9,color:G,fontWeight:600,textAlign:"center"}}>🟢 Live · Supabase</div>}
      ...
      <div style={{flex:1,padding:"22px 26px",overflow:"auto",minWidth:0}} className="fin">
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,paddingBottom:12,borderBottom:`1px solid ${BDR}`}}>
          <div style={{display:"flex",alignItems:"center",gap:9}}>
            ...
            <div><div style={{fontWeight:700,color:INK,fontSize:14}}>{meta.name}</div><div style={{fontSize:10,color:MUTED}}>{meta.industry} · {meta.plan} Plan{user.delegateTo?` · Delegating→${getUser(user.delegateTo)?.name}`:""}</div></div>
          </div>
          <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}>
            ...
            {SB_ENABLED&&<button onClick={loadFromSB} title="Refresh data" style={{background:"none",border:`1px solid ${BDR}`,borderRadius:8,padding:"7px 10px",cursor:"pointer",fontSize:14}}>🔄</button>}
          </div>
        </div>

        {/* TABS */}
        {tab==="dashboard" &&!isManager&&<EmpDash  user={user} myUser={myUser} co={co} setTab={setTab}/>}
        {tab==="dashboard" && isManager&&<MgrDash  co={co} meta={meta} setTab={setTab} getUser={getUser}/>}
        ...
        {tab==="employees"&&hasPerm("manage_employees")&&<Employees companyMeta={meta} users={co.users} setUsers={fn=>{if(!SB_ENABLED)setUsers(fn);}} claims={co.claims} policy={co.policy} toast={toast} addUserToSB={addUserToSB} updateUserInSB={updateUserInSB} sbEnabled={SB_ENABLED}/>}
...
// ─── EMPLOYEES TAB ────────────────────────────────────────────────────────────
function Employees({companyMeta,users,setUsers,claims,policy,toast,addUserToSB,updateUserInSB,sbEnabled}){
  const emps=users.filter(u=>u.role!=="manager");
  const [showAdd,setShowAdd]=useState(false);
  const [del,setDel]=useState(null);
  const [form,setForm]=useState({name:"",email:"",dept:"Audit",balance:"",password:"",role:"employee"});
  const canAdd=emps.length<companyMeta.maxUsers;
  const inpS={padding:"8px 11px",border:`1.5px solid ${BDR}`,borderRadius:8,fontSize:13,background:"#fafff8",fontFamily:FB,width:"100%"};
  const add=async()=>{
    if(!form.name||!form.email||!form.password){toast("Name, email, password required","error");return;}
    if(users.find(u=>u.email===form.email)){toast("Email already exists","error");return;}
    if(!canAdd){toast(`User limit (${companyMeta.maxUsers}) reached`,"error");return;}
    try{
      if(sbEnabled&&addUserToSB){await addUserToSB({name:form.name,email:form.email,role:form.role,dept:form.dept,balance:parseFloat(form.balance)||0},form.password);toast(`✓ ${form.name} added — confirmation email sent`);}
      else{setUsers(p=>[...p,{id:"emp_"+uid(),cid:companyMeta.id,name:form.name,email:form.email,password:form.password,role:form.role,avatar:inits(form.name),dept:form.dept,balance:parseFloat(form.balance)||0,reimbursable:0,delegateTo:null}]);toast(`✓ ${form.name} added`);}
    }catch(e){toast(e.message,"error");return;}
    setForm({name:"",email:"",dept:"Audit",balance:"",password:"",role:"employee"});setShowAdd(false);
  };
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div><h1 style={{fontFamily:FD,fontSize:20,fontWeight:700,color:INK}}>Employees</h1><p style={{color:MUTED,fontSize:11,marginTop:2}}>{emps.length} / {companyMeta.maxUsers} · <span style={{color:canAdd?GD:"#dc2626",fontWeight:600}}>{canAdd?`${companyMeta.maxUsers-emps.length} slots remaining`:"Limit reached"}</span></p></div>
        <Btn onClick={()=>{if(!canAdd){toast(`Limit (${companyMeta.maxUsers}) reached`,"error");return;}setShowAdd(!showAdd);}} v={canAdd?"primary":"outline"}>{showAdd?"✕ Cancel":"＋ Add Employee"}</Btn>
      </div>
      {!canAdd&&<div style={{background:"#fee2e2",border:"1px solid #fca5a5",borderRadius:9,padding:"9px 13px",marginBottom:12,fontSize:12,color:"#dc2626"}}>⚠ User limit of {companyMeta.maxUsers} reached. Contact super admin to increase.</div>}
      {showAdd&&canAdd&&<Card style={{padding:18,marginBottom:12,borderColor:G,background:GL}}>
        <div style={{fontFamily:FD,fontSize:13,fontWeight:700,color:INK,marginBottom:12}}>New Employee</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10}}>
          {[["Full Name *","name","text","Riya Mehta"],["Email *","email","email","riya@co.in"]].map(([l,k,t,ph])=>(
            <div key={k}><label style={{fontSize:9,fontWeight:700,color:MUTED,display:"block",marginBottom:3,textTransform:"uppercase"}}>{l}</label><input type={t} value={form[k]} onChange={e=>setForm({...form,[k]:e.target.value})} placeholder={ph} style={inpS}/></div>
          ))}
          <div><label style={{fontSize:9,fontWeight:700,color:MUTED,display:"block",marginBottom:3,textTransform:"uppercase"}}>Department</label><select value={form.dept} onChange={e=>setForm({...form,dept:e.target.value})} style={{...inpS,appearance:"none"}}>{DEPTS.map(d=><option key={d}>{d}</option>)}</select></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:10,marginBottom:12}}>
          <div><label style={{fontSize:9,fontWeight:700,color:MUTED,display:"block",marginBottom:3,textTransform:"uppercase"}}>Balance ₹</label><input type="number" value={form.balance} onChange={e=>setForm({...form,balance:e.target.value})} placeholder="20000" style={inpS}/></div>
          <div><label style={{fontSize:9,fontWeight:700,color:MUTED,display:"block",marginBottom:3,textTransform:"uppercase"}}>Password *</label><input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="min 6 chars" style={inpS}/></div>
          <div><label style={{fontSize:9,fontWeight:700,color:MUTED,display:"block",marginBottom:3,textTransform:"uppercase"}}>Role</label><select value={form.role} onChange={e=>setForm({...form,role:e.target.value})} style={{...inpS,appearance:"none"}}>{ROLES.map(r=><option key={r.id} value={r.id}>{r.label}</option>)}</select></div>
          <div style={{display:"flex",alignItems:"flex-end"}}><Btn onClick={add} style={{width:"100%",padding:"9px"}}>Add →</Btn></div>
        </div>
      </Card>}
      {del&&<div style={{position:"fixed",inset:0,background:"#00000055",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500}}>
        <div style={{background:"#fff",borderRadius:14,padding:24,width:340,textAlign:"center"}}>
          <div style={{fontSize:28,marginBottom:6}}>⚠️</div>
          <div style={{fontFamily:FD,fontSize:15,fontWeight:700,color:INK,marginBottom:5}}>Remove {del.name}?</div>
          <div style={{color:MUTED,fontSize:12,marginBottom:16}}>Expense history preserved, access revoked.</div>
          <div style={{display:"flex",gap:8,justifyContent:"center"}}><Btn v="danger" onClick={()=>{setUsers(p=>p.filter(u=>u.id!==del.id));setDel(null);toast("Employee removed","warn");}}>Remove</Btn><Btn v="outline" onClick={()=>setDel(null)}>Cancel</Btn></div>
        </div>
      </div>}
      <Card><table style={{width:"100%"}}>
        <thead><tr><th>Employee</th><th>Role</th><th>Dept</th><th>Balance</th><th>Claims</th><th>Adjust</th><th>Delegate</th><th></th></tr></thead>
        <tbody>{emps.map(e=>{
          const ec=claims.filter(c=>c.empId===e.id);
          const role=ROLES.find(r=>r.id===e.role)||ROLES[1];
          return(<tr key={e.id} className="rh">
            <td><div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:28,height:28,borderRadius:"50%",background:GL,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,color:GD,fontSize:10}}>{e.avatar}</div><div><div style={{fontWeight:600,fontSize:12}}>{e.name}</div><div style={{fontSize:10,color:MUTED}}>{e.email}</div></div></div></td>
            <td><span style={{background:role.color+"20",color:role.color,padding:"2px 7px",borderRadius:5,fontSize:10,fontWeight:700}}>{role.label}</span></td>
            <td><span style={{background:GL,color:GD,padding:"1px 6px",borderRadius:4,fontSize:10,fontWeight:600}}>{e.dept}</span></td>
            <td><div style={{fontWeight:700,color:G,fontSize:12}}>{policy.reimbursementMode?fmt(e.reimbursable||0):fmt(e.balance||0)}</div></td>
            <td style={{fontSize:12}}>{ec.length}</td>
            <td>{!policy.reimbursementMode?<input type="number" key={e.id+"b"} defaultValue={e.balance||0} onBlur={ev=>{const val=parseFloat(ev.target.value)||0;if(sbEnabled&&updateUserInSB){updateUserInSB(e.id,{balance:val}).then(()=>toast("Balance updated"));}else{setUsers(p=>p.map(u=>u.id===e.id?{...u,balance:val}:u));toast("Balance updated");}}} style={{width:80,padding:"4px 7px",border:`1.5px solid ${BDR}`,borderRadius:6,fontSize:11}}/>:<span style={{fontSize:10,color:MUTED}}>Reimb.</span>}</td>
            <td><select value={e.delegateTo||""} onChange={ev=>{const val=ev.target.value||null;if(sbEnabled&&updateUserInSB){updateUserInSB(e.id,{delegateTo:val}).then(()=>toast("Delegation updated"));}else{setUsers(p=>p.map(u=>u.id===e.id?{...u,delegateTo:val}:u));toast("Delegation updated");}}} style={{fontSize:11,padding:"4px 7px",border:`1.5px solid ${BDR}`,borderRadius:6,appearance:"none",paddingRight:18}}>
              <option value="">None</option>
              {emps.filter(x=>x.id!==e.id&&["manager","approver"].includes(x.role)).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}
            </select></td>
            <td><button onClick={()=>setDel(e)} style={{background:"none",border:`1px solid #fca5a5`,color:"#ef4444",borderRadius:5,padding:"3px 8px",fontSize:10,cursor:"pointer"}}>Remove</button></td>
          </tr>);
        })}</tbody>
      </table></Card>
    </div>
...
function Policy({policy,setPolicy,savePolicy,toast,users,sbEnabled}){
  ...
  return(
    <div>
      ...
      <div style={{marginTop:12}}><Btn onClick={async()=>{if(sbEnabled&&savePolicy){try{await savePolicy(policy);toast("✓ Settings saved to database");}catch(e){toast(e.message,"error");}}else{toast("✓ Settings saved");}}}>Save All Settings</Btn></div>
    </div>
  );
}
    ...
    <div style={{background:"#fff",borderRadius:inline?14:18,width:inline?"100%":720,maxHeight:inline?"none":"88vh",display:"flex",overflow:inline?"visible":"hidden",boxShadow:inline?"none":"0 24px 60px #0004",flexDirection:inline?"column":"row"}}>
      ...
      {!inline&&<div style={{width:220,background:DARK,flexShrink:0,display:"flex",flexDirection:"column",padding:"24px 12px"}}>
        ...
        <div style={{marginTop:"auto",paddingTop:12,borderTop:"1px solid rgba(255,255,255,.1)"}}>
          <div style={{fontSize:10,color:"rgba(255,255,255,.3)",textAlign:"center",lineHeight:1.5}}>HisaabHub v5<br/>by RB · support@hisaabhub.in</div>
        </div>
      </div>}
      {/* Content */}
...
export default function Root(){
  ...
  const[session,setSession]=useState(null);        // {userId, companyId, role}
  const[loading,setLoading]=useState(true);

  // Persist localStorage DB whenever it changes (demo mode only)
  useEffect(()=>{ if(!SB_ENABLED)saveDB(DB); },[DB]);

  useEffect(()=>{
    if(SB_ENABLED){
      // ── Supabase: check existing session on mount ─────────────────────
      supabase.auth.getSession().then(async({data:{session:sess}})=>{
        if(sess?.user)await resolveSupabaseUser(sess.user.id);
        else setLoading(false);
      });

      // ── Supabase: listen for login/logout/OAuth callback ──────────────
  ...
  const resolveSupabaseUser=async(authUserId)=>{
    try{
      ...
      const{data:saRow}=await supabase.from("super_admins").select("id").eq("id",authUserId).maybeSingle();
      if(saRow){
        const{data:{user}}=await supabase.auth.getUser();
        setSession({userId:authUserId,companyId:null,role:"superadmin",sbUser:{id:authUserId,name:"Super Admin",email:user?.email||"admin@hisaabhub.in",role:"superadmin",avatar:"SA"}});
        setLoading(false);return;
      }
      // Regular user — get profile
      const{data:profile,error}=await supabase.from("users").select("*").eq("id",authUserId).single();
      if(error||!profile){setLoading(false);return;}
      setSession({userId:authUserId,companyId:profile.company_id,role:profile.role,sbUser:mapUser(profile)});
    }catch(e){console.error("Auth resolve error:",e);}
    finally{setLoading(false);}
  };

  const handleLogin=(u,m)=>{
    // Called from Login in demo mode only
    if(u.role==="superadmin"){
      const s={userId:"sa1",companyId:null,role:"superadmin"};
      setSession(s);saveSess(s);
    } else {
      const s={userId:u.id,companyId:m?.id||null,role:u.role};
      setSession(s);saveSess(s);
    }
  };
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:`linear-gradient(145deg,${DARK},#162e0d)`,fontFamily:FB}}>
      <div style={{textAlign:"center"}}>
        ...
        <div style={{marginTop:24,display:"flex",alignItems:"center",justifyContent:"center",gap:8,color:"rgba(255,255,255,0.4)",fontSize:13}}>
          <span style={{width:18,height:18,border:"2px solid rgba(255,255,255,0.2)",borderTopColor:G,borderRadius:"50%",animation:"spin .8s linear infinite",display:"inline-block"}}/>
          {SB_ENABLED?"Connecting to Supabase…":"Loading…"}
        </div>
      </div>
    </div>
  );
  ...
  // Session exists but user not found → clear and show login
  if(session&&!currentUser){
    saveSess(null);
    return<ErrorBoundary><Login onLogin={handleLogin} DB={DB}/></ErrorBoundary>;
  }
  ...
  if(currentUser.role==="superadmin")
    return<ErrorBoundary><SuperAdmin DB={DB} setDB={setDB} onLogout={handleLogout}/></ErrorBoundary>;

  // For Supabase mode, CompanyApp fetches its own data — pass minimal meta
  if(SB_ENABLED){
    return<ErrorBoundary>
      <CompanyApp
        user={currentUser}
        meta={{id:session.companyId,...currentMeta}}
        DB={DB}
        setDB={setDB}
  ...
  return<ErrorBoundary>
    <CompanyApp user={currentUser} meta={currentMeta} DB={DB} setDB={setDB} onLogout={handleLogout}/>
  </ErrorBoundary>;
}
